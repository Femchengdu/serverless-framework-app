...
This project implements the following functions and configures them in the `serverless.yml` file:
...
All necessary resources to the `resources` section of the `serverless.yml` file such as DynamoDB table and S3 bucket.

# Frontend

The `client` folder contains an example web application that can use the API.

This frontend should work with your serverless application once you edit the `config.ts` file in the `client` folder. This file configures your client application and contains an API endpoint and Auth0 configuration:
...
To store TODO items a DynamoDB table with local secondary index(es) is used like this:
